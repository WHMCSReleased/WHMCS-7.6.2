...
<div id="order-modern">
    ...
    {if $cartitems!=0}
        ...
        <form method="post" action="{$smarty.server.PHP_SELF}?a=checkout" id="frmCheckout">
            ...
            <div align="center">
                <button type="submit" id="btnCompleteOrder"{if $cartitems==0} disabled{/if} onclick="this.value='{$LANG.pleasewait}'" class="btn btn-primary btn-lg disable-on-click spinner-on-click" {if $custtype eq "existing" && !$loggedin}formnovalidate{/if}>
                    {$LANG.checkout}
